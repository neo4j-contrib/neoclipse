...
public class Activator extends AbstractUIPlugin {
		...
		public void run() {
			...
			while (!stop) {
				...
				synchronized(Activator.this) {
					if (stop)
						break;
					long timeout = Activator.getIntPref(IConstants.CLOSE_UNUSED_CONNECTIONS_AFTER);
					if (timeout < 1)
						continue;
    ...
    public void startDefaultConnections(ConnectionsView connectionsView) {
        ...
        boolean openEditor = Activator.getBooleanPref(IConstants.AUTO_OPEN_EDITOR);
        ...
        for (Alias alias : aliasManager.getAliases()) {
            if (alias.isConnectAtStartup() && alias.isAutoLogon() && alias.getDefaultUser() != null) {
                ...
                if (openEditor) {
                    SQLEditorInput input = new SQLEditorInput("SQL Editor (" + Activator.getDefault().getEditorSerialNo() + ").sql");
                    input.setUser(alias.getDefaultUser());
                    try {
                    	site.getPage().openEditor(input, SQLEditor.class.getName());
                    }catch(PartInitException e) {
                    	Activator.error("Cannot open SQL editor", e);
                    }
                }
            }
        }
