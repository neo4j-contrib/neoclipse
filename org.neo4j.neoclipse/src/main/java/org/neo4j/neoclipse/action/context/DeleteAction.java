...
public class DeleteAction extends ContextAction
{
    ...
    /**
     * @todo nodes get out of sight when the relationship they hang on to is
     *       deleted.
     */
    @Override
    public void run()
    {
        PropertyContainer container = getPropertyContainer();
        if ( container == null )
        {
            return;
        }
            ...
            if ( container instanceof Node )
            {
                ...
                if ( ns.getReferenceNode().equals( node ) )
                {
                    MessageDialog.openError( null, "Error",
                        "Deleting the reference node is not allowed." );
                    return;
                }
